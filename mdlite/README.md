...
- 斜杠命令（Notion 风格）：输入 `/` 在光标右下方弹出块命令菜单——任务/无序/有序
...
- 知识图谱：`Ctrl+G` 进入/退出全库链接关系图（支持 `[[wiki]]` 和 `[text](target)` 两种语法，
  全盘扫描不依赖文件树展开状态）；力导向布局、拖拽节点、双击跳转；
  hover 节点显示信息卡（链入/链出统计与高亮描边），点击聚焦并淡出无关节点，
  空白处拖拽平移、滚轮以光标为锚点缩放；节点右上角显示连接数 badge，
  右上角统计徽章（笔记/链接/孤儿数）；Esc 分层取消（先清选中、再退出图谱）；
  保存文档后自动重建
